...
import { stepConfigs } from "@/lib/steps";
import { StepConfig } from "@/lib/types";
import { sampleDocs } from "@/lib/utils/sampleDocs";
import { canAccessStep } from "@/lib/utils/stepAccess";
import JSZip from "jszip";
import Footer from "../components/Footer";
import WizardProgress from "./components/WizardProgress";
import { Terminal, ChevronRight, Check, Download, RotateCcw, FileJson, X, AlertTriangle } from 'lucide-react';
        ...
        <aside className="space-y-6 flex flex-col">
          ...
          <div className="bg-zinc-900 border border-zinc-800 p-6 lg:sticky lg:top-20 order-1 lg:order-2 animate-fadeSlideUp animate-delay-250">
            ...
            <button
              ...
              disabled={
                isGenerating ||
                (currentStep === 1 && !steps[currentStepKey].chatHistory.some(msg => msg.role === 'user'))
              }
            ...
            {steps[currentStepKey].generatedDoc && 
             steps[currentStepKey].lastGeneratedMessageCount !== undefined && 
             steps[currentStepKey].chatHistory.length > steps[currentStepKey].lastGeneratedMessageCount! && (
              <div className="mb-3 px-3 py-2 bg-amber-950/20 border border-amber-900/50 rounded flex items-start gap-2 animate-pulse">
                <AlertTriangle className="w-4 h-4 text-amber-500 shrink-0 mt-0.5" />
                <p className="text-[10px] text-amber-200/80 leading-relaxed font-mono">
                  YOU_HAVE_NEW_DETAILS. REGENERATE TO INCLUDE THEM IN THE SPEC.
                </p>
              </div>
            )}
